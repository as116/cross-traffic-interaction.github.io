    ...
    data.forEach(entry => {
        ...
        for (const key in entry) {
            ...
            if (typeof entry[key] === 'string' && entry[key].includes(', ')) {
                const values = entry[key].split(', ').map(value => value.trim());
...
function filterAndDisplayData() {
    ...
    if (activeFilters.length > 0) {
        filteredData = jsonData.filter(entry => {
            ...
            var trafficDensityA = entry['Situation - Traffic Density A'].toLowerCase();
            var laneNumberA = entry['Situation - Lane Number A'].toLowerCase();
            var trafficAutonomy = entry['Situation - Traffic Autonomy A'].toLowerCase();
...
function displayData() {
    ...
    filteredData.forEach((entry, index) => {
        ...
        titleButton.setAttribute('id', entry['Article ID']);
        titleButton.textContent = entry['Article Title'];
        ...
        for (var key in entry) {
            if (key !== 'Article ID' && key !== 'Resolve?'  && key !== 'User Notes' && key !== 'Notes' && key !== 'Status' && key !== 'User Count' && key !== 'User Name' && key !== 'Include' && key !== 'Article Title') {
    ...
    entryCount.textContent = 'Papers found: ' + filteredData.length;
...
function createParallelCategories() {
    ...
    var lanes = filteredInteractionData.map(entry => entry['Situation - Lane Number A']);
    var setting = filteredInteractionData.map(entry => entry['Situation - Lane Setting A']);
    var density = filteredInteractionData.map(entry => entry['Situation - Traffic Density A']);
    var autonomy = filteredInteractionData.map(entry => entry['Situation - Traffic Autonomy A']);
    ...
    var update_color = function (points_data, containerID) {
        ...
        for (var i = 0; i < points_data.points.length; i++) {
            ...
            console.log(clickedData['Article ID']);
            var articleID = clickedData['Article ID'];
            ...
            document.getElementById(articleID).style.backgroundColor = containerID === 'S-container' ? highlightColorS : highlightColorIR;
        ...
        .on('click', function (d) {
            ...
            filteredInteractionData.forEach((entry, index) => {
                ...
                if (ipA === d.name.trim() ||
                    ...
                    var articleID = entry['Article ID'];
                    document.getElementById(articleID).style.backgroundColor = "#007bff";
                }
            });
